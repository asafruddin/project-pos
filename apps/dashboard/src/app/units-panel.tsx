...
import type { ApiErrorBody, UnitListResponse, UnitRecord } from "@pos-apps/types";
...
export function UnitsPanel({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows((data as UnitListResponse).units);
      setError(null);
    } catch {
    ...
    <div className="flex min-w-0 flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {loading ? "Memuat…" : `${rows.length} satuan`}
          </p>
        </div>
        {canCreate ? <CreateLink href="/units/new">Tambah satuan</CreateLink> : null}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-dashed border-border bg-secondary/40 px-4 py-10 text-center">
          <p className="text-sm text-muted-foreground">Belum ada satuan.</p>
        </div>
      ) : (
        <>
          <ul className="grid gap-3 sm:hidden">
            {rows.map((row) => (
              ...
              >
                ...
                {canEdit ? (
                  <div className="mt-3">
                    <RowLink href={`/units/${row.unit_id}/edit`}>Ubah</RowLink>
                  </div>
                ) : null}
              </li>
            ))}
