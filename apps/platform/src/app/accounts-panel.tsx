...
export function AccountsPanel() {
  ...
  async function patchUser(userId: string, body: { active?: boolean }) {
    ...
    setError(null);
    try {
      const res = await authorizedFetch(`/platform/accounts/${userId}`, {
        method: "PATCH",
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <CreateLink href="/accounts/new">Tambah akun</CreateLink>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-dashed border-border bg-secondary/40 px-4 py-10 text-center">
          <p className="text-sm text-muted-foreground">Belum ada akun POS.</p>
        </div>
      ) : (
        <>
            ...
            {users.map((row) => (
              ...
              >
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <RowLink href={`/accounts/${row.user_id}/edit`}>Ubah</RowLink>
                  <Button
                    type="button"
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          <RowLink href={`/accounts/${row.user_id}/edit`}>
                            Ubah
                          </RowLink>
                          <Button
                            type="button"
